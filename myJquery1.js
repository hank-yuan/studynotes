(function (window, undefined) {
  ...
  var parseHtml = (function () {
    ...
    function parseHtml (html) {
      var res = []
      // 这边是将具有html语义的字符串放在模板div中,div里面的就有这些标签了
      div.innerHTML = html
  ...
  cc.fn = cc.prototype = {
    ...
    init: function (html) { // 这个函数的作用就是:1.创建实例;2.在这个函数里面对所传入的参数进行一个判断
      ...
      if (html == '') { // 这边判断传入的参数是不是一个空字符串
        // 如果传入的是一个空字符串，那么就直接返回this，注意，这边虽然没写返回this,但是他是默认的
...
var deltaTime = currentTime - startTime //当前时间与开始事件的差值,这个差值可以得到的是已经过去了多久
